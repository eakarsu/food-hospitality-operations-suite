...
const adminCookie = await login('admin@food-hospitality.local', 'admin123');
const managerCookie = await login('manager@food-hospitality.local', 'manager123');
const analystCookie = await login('analyst@food-hospitality.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/locations', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/locations', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/locations/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/locations/approve`, {
  method: 'POST',
  headers: {
